<?
	...
	$login=$_SESSION['login'];
      ...
      select_1_row("select setting_autochange,level,bought_invisibility,setting_invite_sound,setting_full_buttons,last_channel,ul_mode_contacts_only,ul_mode_hide_female,ul_mode_hide_male from tbl_users where id=$uid");
      $has_inv=select_1("select invisibility from levels where id=$level")+$bought_invisibility;
...
var obutt_size=60;
var console_size=50;
var ccount=<?echo select_1("select count(*) from channels");?>;
var in_ul=0;
...
function UserinfoID(uid)
{
	...
	locat='/chat_userinfo2.php?id='+uid;
...
function userinfoNI(nick)
{
	r=Math.round(Math.random(123)*1000);
	{
//	id=FindIDByNick(nick);
		locat='/chat_userinfo2.php?ni='+escape(nick);
		open(locat,'uinfo'+r,'toolbar=0,status=0,resizable=1,scrollbars=1,width=1000,height=800');
	}
	return false;
}
