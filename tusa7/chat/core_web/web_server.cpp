...
void *web_server::new_connection(const PTR_getter<c_socket>&s)
{
    set<string> sss;
    return new web_connection(this,s);
}
...
void web_server::load_config(const std::string& pn,const std::string& prefix,std::map<std::string,std::string>&m)
{
    ...
    set_cfg_i(m,	prefix+"inspector_time_usec", pn,inspector_time_usec,1000000);

    set_cfg_str(m,	prefix+"adminfo_url", pn,adminfo_url,"/ainf");
    ...
    {
        string a=get_cfg_str(m,prefix+"replacement_config",pn, "./conf/replacement.conf");
...
map<string,string> web_server::get_replacements()
{
    M_LOCK(replacementsMX);
    map<string,string> ret;
    for (map<string,vector<string> >::iterator i=replacements.begin();i!=replacements.end();i++)
    {
        if (i->second.size()==1)
        {
