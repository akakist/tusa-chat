...
void irc_send_welcome(const user_pair&__U)
{
    ...
    for (unsigned int i=0;i<v.size();i++) {
        ...
        PGLW(chat_user, z,v[i]);
        if (z->cstate.channel.size())
        {
            if (z->user_type==TYPE_WWW)
                from_web++;
            if (z->user_type==TYPE_IRC)
                from_irc++;
            total++;
        }
    ...
    irc_send_reply(__U,254,itostring(chat_channels.keys().size()),string(irc_server_config.msg254));
    char ss[300];
    snprintf(ss,sizeof(ss)-1,string(irc_server_config.msg255).c_str(),all_u,total,from_web,from_irc);
    ...
    if (load_1_file(motd,string(irc_server_config.motd))==-1) {
        ...
        data_push_back(__U.cu,motd);
        irc_send_reply(__U,376,"End of MOTD command.");
    }
    logErr2("%s (%s) logged in from IRC [%s].",GET_CNICK(__U).c_str(),user$login(__U).c_str(),user$ip(__U).c_str());
}

void irc_send_notice(const user_pair&__U,const string &msg,const string &from)
{
    string s;
    s<<":"<<from<<"!"<<from<<"@"<<string(irc_server_config.host)<<" NOTICE "<<GET_CNICK(__U)<<" :"<<msg<<"\r\n";
    data_push_back(__U.cu,s);
}
void irc_send_notice(const PTR_getter<chat_user>&__U,const string &msg,const string &from)
{
    string s;
    s<<":"<<from<<"!"<<from<<"@"<<string(irc_server_config.host)<<" NOTICE "<<irc_NICK(__U)<<" :"<<msg<<"\r\n";
...
void irc_send_private_message(const user_pair&__U,const string &msg,const string &from)
{
    string ms=msg;
    string s;
    s<<":"<<from<<"!"<<from<<"@"<<string(irc_server_config.host)<<" PRIVMSG "<<GET_CNICK(__U)<<" :";
    if (ms.find("<br>")!=-1) {
        ms=str_replace("<br>","\r\n"+s,msg);
    }
    s<<ms<<"\r\n";
...
void irc_send_private_message(const PTR_getter<chat_user>&__U,const string &msg,const string &from)
{
    string ms=msg;
    string s;
    s<<":"<<from<<"!"<<from<<"@"<<string(irc_server_config.host)<<" PRIVMSG "<<irc_NICK(__U)<<" :";
    if (ms.find("<br>")!=-1) {
        ms=str_replace("<br>","\r\n"+s,msg);
    }
    s<<ms<<"\r\n";
...
string s_irc_send_action_to_channel(const string &msg,const string &from, const string& chan)
{
    char cc1=1;
    string s;
    s<<":"<<from<<"!"<<from<<"@"<<string(irc_server_config.host)<<" PRIVMSG "<<chan<<" :";
    s+=cc1;
    s<<"ACTION "<<msg;
    s+=cc1;
    s<<"\r\n";
...
{
    string s;
    s<<":"<<from<<"!"<<from<<"@"<<string(irc_server_config.host)<<" PRIVMSG "<<channel<<" :"<<msg<<"\r\n";
    return s;
}
...
void irc_send_notice(const PTR_getter<irc_session>&__U,const string &msg,const string &from)
{
    string s;
    {
        ...
        s<<":"<<from<<"!"<<from<<"@"<<string(irc_server_config.host)<<" NOTICE "<<u->nick<<" :"<<msg<<"\r\n";
    }
    data_push_back(__U,s);
