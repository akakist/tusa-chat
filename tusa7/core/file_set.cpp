...
void params_map::read(const string&fn)
{
    ...
    if (load_1_file(fbody, fn) == -1) {
        logErr2("--Error: reading config file %s (%s)! %s", fn.c_str(), strerror(errno),"");
    }
    string c = fbody;
...
void params_map::print()
{
    ...
    {
        M_LOCK(Mutex);
        for (map<string,string>::iterator i = container->begin(); i != container->end(); i++) {
...
string params_map::get(const char *s)
{
    ...
    if (!res) {
        string r = (string) "Unable get value by key " + s;
        printf("%s\n", r.c_str());
        throw cError(r);
    }
    return ret;
