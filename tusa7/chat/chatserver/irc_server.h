...
public:
    ...
    void process_command(const string& cmd,const PTR_getter<tcp_second_data>& __SD);
    void on_delete(const PTR_getter<tcp_second_data>& __SD,const string& msg);

    void on_accept_dialog_socket(const PTR_getter<c_socket>&s,tcp_second_data*) {}
    bool validate_second_data(const PTR_getter<tcp_second_data>&,string& errstr);
...
public:
    ...
    ~irc_connection();
    tcp_second_data* new_tcp_second_data(tcp_dialog_server* serv) {
        return new tcp_second_data(serv);
    }
};
