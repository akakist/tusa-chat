...
void user_status(cMYSQL * dbh)
{
    ...
    vector<vector<string> > v=dbh->execQ((QUERY)"select id,uid,name,pic,disable_call,uniq from ra_old.user_status where uid<'?'"<<LIMIT);
...
{
    printf("\nnews...\n");
    vector<vector<string> > v=dbh->execQ((QUERY)"select id,add_nid from ra_old.news");
    for(unsigned i=0;i<v.size();i++)
    {
        if(v[i].size()==2)
        {
            ...
            string n=dbh->select_1Q((QUERY)"select ra_old.nicks.nick from ra_old.nicks where id=?"<<v[i][1]);
            dbh->real_queryQ((QUERY)"update ra.news set ra.news.add_by='?' where ra.news.id=?"<<n<<v[i][0]);
        }
    }
...
void nicks(cMYSQL *dbh)
{
    ...
    try{
        vector<vector<string> >v=dbh->execQ((QUERY)"select id,login,nick, uppernick,banned,uid,bby from ra_old.nicks where uid<'?'"<<LIMIT);
...
void note_folders(cMYSQL *dbh)
{
    ...
    vector<vector<string> > v=dbh->execQ((QUERY)"select id,uid,name from ra_old.notes_folders where uid<'?'"<<LIMIT);
    for(unsigned i=0;i<v.size();i++)
    {
        if(v[i].size()==3)
        {
            dbh->real_queryQ((QUERY)"insert into ra.note_folders (id,uid,name) values ('?','?','?')"<<v[i][0]<<v[i][1]<<v[i][2]);
            if(i%1000==0)printf("notefolders %d (%d)\n",i,v.size());
        }
        else printf("v[i].size()==3\n");
    }
    vector<vector<string> > vn=dbh->execQ((QUERY)"select id,folder from ra_old.notes where uid<?"<<LIMIT);
...
void note_texts(cMYSQL *dbh)
{
    ...
    vector<vector<string> > v=dbh->execQ((QUERY)"select id,uid,msg from ra_old.notes where uid<?"<<LIMIT);
    for(unsigned i=0;i<v.size();i++)
    {
...
void notes(cMYSQL *dbh)
{
    ...
    dbh->real_query("CREATE TABLE `notes` ( \
                    ...
                    ");
    vector<string> a=dbh->select_1_columnQ((QUERY)"select uid from ra_old.notes where uid<? group by uid"<<LIMIT);
    for(unsigned i=0;i<a.size();i++)
    {
        ...
        vector<vector<string> > v=dbh->execQ((QUERY)"select id,uid,from_nick,from_nick_id,to_nick,to_nick_id,subj,\
                                             ...
                                             file_size,unread from ra_old.notes where uid=?"<<a[i]);
        for(unsigned i=0;i<v.size();i++)
        {
            if(v[i].size()!=21)
            {
...
{
    string pn=cache_dir("user",uid.container)+"/photos" SPLITTER+itostring(pid)+"."+ext;
    return pn;
}
...
void photos(cMYSQL *dbh)
{
    ...
    vector<vector<string> >v=dbh->execQ((QUERY)"select id,filename,orig_filename,descr,sx,sy,uid,size from ra_old.photos where uid<?"<<LIMIT);
    for(unsigned i=0;i<v.size();i++)
    {
        printf("photo %d (%d)\n",i,v.size());
        if(v[i].size()==8)
...
void users(cMYSQL *dbh)
{
    ...
    */
    ...
    vector<vector<string> > vuid=dbh->execQ((QUERY)"select \
                                            ...
                                            from ra_old.users where id<?"<<LIMIT);
    for(unsigned i=0;i<vuid.size();i++)
    {
        ...
        vector<string> vs=dbh->select_1_rowQ((QUERY)
                                             ...
                                             notes_sent_folder,notes_msg_per_page,notes_sort_by,notes_save_copy,n_color_pack,settings from ra_old.user_settings\
...
#define h_icq		128
        ...
        dbh->real_queryQ((QUERY)"insert into user_profiles\
                         ...
                         b_day,b_mon,b_year,fname,lname,email,reg_date,level,login,pass,vote_balls,\
                         db_privileges,upperlogin,gender,has_clan)\
                         values(\
                         '?','?','?','?','?','?','?',\
                         '?','?','?','?','?','?','?',\
                         '?','?','?','?','?','?','?'\
                         ...
                         <<BDay<<BMonth<<BYear<<firstname<<lastname<<email<<RegDate<<accessmode*10<<login<<pass<<vote_balls
                         <<0<<str_nick(login)<<gender<<0
                        );
        string lnick=dbh->select_1Q((QUERY)"select nick from ra_old.nicks where id=?"<<anick);
        dbh->real_queryQ((QUERY)"insert into user_sets\
                         ...
                         setting_show_stat,setting_invite_sound,setting_full_buttons,hide_fname,\
                         hide_lname,hide_bdate,hide_tel,hide_icq,\
                         hide_email)\
                         values \
                         (\
                         '?','?','?','?',\
                         ...
                         <<(settings&setting_show_stat)<<(settings&setting_invite_sound)<<(settings&setting_full_buttons)<<!(Public&h_fname)
                         <<!(Public&h_lname)<<!(Public&h_date)<<!(Public&h_tel)<<!(Public&h_icq)
                         <<!(Public&h_email)
                        );

        dbh->real_queryQ((QUERY)"insert into user_stats\
                         (refid,last_date,last_ip,v_count,\
                         m_count,t_count,kick_count,kicked_count,last_pip)\
                         values(\
                         '?','?','?','?',\
                         '?','?','?','?','?'\
                         )"
                         <<id<<LastDate<<LastIP<<VisCount
                         <<messagecount<<timecount<<kick<<kicked<<""
                        );

        int fld_in=atoi(dbh->select_1Q((QUERY)"select to_folder from ra_old.notes_filters where fid=0 and uid=?"<<id));
        //notes_sent_folder
...
int main(int argc,char *argv[])
{
	...
	printf("\n%s\n",dn);
        MYSQL *res=mysql_real_connect(_dbh,NULL, 
	...
	0);
        if(!res)
        {
            logErr("chat_sender: mysql_real_connect failed to %s","ra");
            exit(1);
        }
