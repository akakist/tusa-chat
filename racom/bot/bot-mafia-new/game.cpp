...
//	int maf=(sz*25/100);
    ...
    while (left>0) {
        if (left>0) {
            if (doc>0 && sz<30*doc) {} else {
                doc++;
                left--;
            }
        }
        if (left>0) {
            if (man>0 && sz<10*man) {} else {
                man++;
                left--;
            }
        }
        if (left>0) {
            if (bom>0 && sz<15*man) {} else {
                bom++;
                left--;
            }
        }
        if (val>0 && left>0) {
            if (kat>0 && sz<20*kat) {}else {
                kat++;
                left--;
            }
        }
        if (left>0) {
            if (val>0 && sz<30*val) {} else {
                val++;
                left--;
            }
        }
...
void process_game()
{
    ...
    if (roles_assign && !roles_assign_half && ct-roles_assign_time>ROLE_ACCEPT_TIME/2) {
...
//			if(u->marked_for_delete) continue
    ...
    if (roles_assign && ct-roles_assign_time>ROLE_ACCEPT_TIME) {
        check_roles();
        roles_assign=false;
        make_main_maf(true);
    ...
    if (night && ct-night_time>NIGHT_TIME) {
        night=false;
        do_day();
    }
    if (morning && ct-morning_time>MORNING_TIME) {
        morning=false;
        do_day();
    }
    if (day) {
        ...
        if (vote1 && ct-vote1_time>VOTE1_TIMEOUT) {
            end_vote1();
            vote1=false;
        }
        if (vote_say && ct-vote_say_time>VOTE_SAY_TIMEOUT) {
            do_vote2();
            vote_say=false;
        }
        if (vote2 && !vote2_half && ct-vote2_time>VOTE2_TIMEOUT/2) {
            invite_vote2();
            vote2_half=true;
        }
        if (vote2 && ct-vote2_time>VOTE2_TIMEOUT) {
            end_vote2();
            vote2=false;
        }
    }
...
{
    if (gamers_by_nick.find(str_nick(n))!=gamers_by_nick.end()) {
    ...
    bool allacc=true;
    for (map<string,c_user*>::iterator i=gamers_by_nick.begin();i!=gamers_by_nick.end();i++) {
        if (i->second->marked_for_delete) continue;
...
//	}
    ...
    if (rls[ROLE_MAF].size()==0 && rls[ROLE_MAN].size()==0) {
        winner=ROLE_MIR;
        end=true;
    }
    if (rls[ROLE_MAF].size()>=tmir && tmir<3 && rls[ROLE_KAT].size()==0) {
        winner=ROLE_MAF;
        end=true;
    }
    if (tmir<2 && rls[ROLE_MAN].size()>0 && rls[ROLE_MAF].size()==0 && rls[ROLE_KAT].size()==0) {
        end=true;
        winner=ROLE_MAN;
    }
    if (rls[ROLE_MAF].size()==1 && rls[ROLE_KAT].size()==1 && tmir==1) {
        end=true;
        draw=true;
    }
    if (rls[ROLE_MAF].size()==1 && rls[ROLE_MAN].size()==1 && tmir<2) {
        end=true;
        draw=true;
    }
    if (rls[ROLE_MAN].size()==1 && rls[ROLE_KAT].size()==1 && tmir==1 && tbad==1) {
        end=true;
        draw=true;
    }
...
//		sleep(1);
        if (draw) {
            ...
            if (winner==ROLE_MAF || winner==ROLE_MAF) {
                sc[ROLE_MAF]=50;
                sc[ROLE_MAN]=50;
            } else {
                sc[ROLE_MIR]=50;
        ...
        for (map<int,bool>::iterator i=sr.begin();i!=sr.end();i++) {
...
//		sleep(1);
        ...
        action(_MSG("roles"));
        sleep(1);
        for (map<unsigned int,vector<c_user*> >::iterator i=rlsabs.begin();i!=rlsabs.end();i++) {
            if (i->second.size()>0) {
                string ss;
