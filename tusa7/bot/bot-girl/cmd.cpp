...
user::user():last_time(0),cnt(0),ignored(false),ig_cnt(0),cnt_per5(0),last_time_per5(0) {
    max_cnt=30+(rand()%100);
};
    ...
    if (w.size()>5) {
        if (
            ...
            if (msg.find("!!!")==-1 && msg.find("???")==-1 && msg.find(")))))")==-1 && msg.find("(((((")==-1 && msg.find("   ")==-1)
                dbh->real_queryQ((QUERY)"insert into unknow (msg) values ('?')"<<cmd.msg);
...
//	printf("tm=%d c=%d\n",time(NULL)-u->last_time_per5,u->cnt_per5);
    vector<string> v=dbh->select_1_columnQ((QUERY)"select answer from msg where '?' REGEXP pattern"<<cmd.msg);
    if (v.size()) {
        int r=rand()%v.size();
        if (r<v.size()) {
...
void add_greeting(c_command &cmd)
{
    ...
    if (pz!=-1) {
        ...
        dbh->real_queryQ((QUERY)"insert into greetingmsg (answer) values ('?')"<<msg);
...
//	if(!worked) return;
    ...
    if (cmd.cmd=="PRIVMSG") {
        ...
        if (cmd.user=="Arsenic"||cmd.user=="Mazar"||cmd.user=="KPOXA" ||cmd.user=="mumik"||cmd.user=="Lena") {
            if (strex(msg,"!start")) {
                worked=true;
