...
void out_oscar_buffer::Pack(const char * s, int len)
{
    adjust(len);
...
void out_oscar_buffer::clear(){cur_pos=0;}
...
out_oscar_buffer::out_oscar_buffer():buffer(NULL), bufsize(0),cur_pos(0)
{
        ...
        if(buffer==NULL) throw cError((string)"alloc error");
        bufsize=256;
        cur_pos=0;
...
void out_oscar_buffer::adjust(int n)
{

    if(cur_pos+n>=bufsize)
    {
        int nnew=cur_pos+n+1024;
        buffer=(unsigned char*)realloc(buffer,nnew);
        if(!buffer) throw cError((string)"alloc error");
        bufsize=nnew;
    }
...
oscar_buffer& oscar_buffer::operator>>(signed char& p)
{
    p=get_PN();
    return *this;
}
...
out_oscar_buffer& out_oscar_buffer::operator<<(signed char c)
{
    put_PN(c);
    return *this;
}
...
out_oscar_buffer& out_oscar_buffer::operator<<(signed short c)
{
    put_PN(c);
    return *this;
}
...
out_oscar_buffer& out_oscar_buffer::operator<<(signed int c)
{
    put_PN(c);
    return *this;
}
...
unsigned int out_oscar_buffer::size() const
{
    return cur_pos;
}
