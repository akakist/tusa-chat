...
void check_for_flood(c_command &cmd)
{
    ...
    if (last_words.find(cmd.user)!=last_words.end()) {
        string lw=last_words[cmd.user].s;
        int len=lw.size()-2;
        if (len<=lw.size()) len=lw.size()-1;
        if (msg.size()<2) len=msg.size();
        if (len>0) {
            if (msg.substr(0,len)==lw.substr(0,len)) {
                if (last_words[cmd.user].cnt>4) {
                    last_words.erase(cmd.user);
                    check_for_ban(cmd.fromnick,cmd.user);
                    sock.response("KICK "+params["CHANNEL"]+" "+cmd.fromnick+" :флуд");
                    return;
                } else {
                    last_words[cmd.user].cnt++;
                }
            }
//			else{
...
//			}
    ...
    last_words[cmd.user].last_time=time(NULL);
    last_words[cmd.user].s=msg;
