...
if(isset($arg_delete))
    {
    mysql_query("delete from news where id='".mysql_escape_string($arg_delete)."'");
    $tmp = "DEL_NEWS IP $ip ";
    mysql_query("insert into adm_events (login,date_events,type_events,events) values('".mysql_escape_string($login)."','$d','DELNEWS','$tmp')") or die(mysql_error());
    }
if(isset($arg_new))
    {
    mysql_query("insert into news (add_date,txt,add_by) values (NOW(),'".mysql_escape_string($arg_new)."','".mysql_escape_string($arg_selectnick)."')") or die(mysql_error());
    $tmp = "NEWS IP $ip ";
