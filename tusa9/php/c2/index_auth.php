<?
	...
	{
			$res=mysql_query("select priv from tbl_privs where uid=".intval($uid)) or die(mysql_error().__FILE__.__LINE__);
			$privs=array();
	...
	{
			$res=mysql_query("select channel from moderators where uid=".intval($uid)) or die(mysql_error().__FILE__.__LINE__);
			$m=array();
			while($rw=mysql_fetch_row($res))
			{
				$m[$rw[0]]="1";
	...
	list($time_cnt,$time_free,$descr)=mysql_select1row("select time_cnt,time_free,descr from banned_login where uid=".intval($uid)." and time_free>NOW()");
	if(isset($time_cnt))
	{
	...
	if(!isset($error))
	    ...
	    $sid=md5(session_id());
	    $oldsid=mysql_select1("select  php_session_id from tbl_users where id=".intval($uid));
	    if($sid!=$oldsid)
	    {
	 	   mysql_query("update tbl_users set php_session_id='".mysql_escape_string($sid)."' where id=".intval($uid)) or die(mysql_error());
	    }
	}
