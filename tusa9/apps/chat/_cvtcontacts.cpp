...
//	FILE *f=fopen("clear_users.log","a");
	...
	vector<vector<string> > v=dbh->exec("select refid,contacts from user_sets");
	    printf("/*!40000 ALTER TABLE `contact` DISABLE KEYS */;\n");
	...
	if(v[i].size()==2)
	{
	    ...
	    set<unsigned int> s;
	    b>>s;
	    for(typeof(s.begin())j=s.begin();j!=s.end();j++)
	    {
		printf("insert into contact (uid,cuid) values('%s','%d');\n",v[i][0].c_str(),*j);
	    }
...
//	    printf("update nicks set ready_to_sale='1' where id=%s; # %s (%s)\n",v[i][0].c_str(),v[i][1].c_str(),v[i][2].c_str());
    ...
    printf("/*!40000 ALTER TABLE `contact` ENABLE KEYS */;\n");
