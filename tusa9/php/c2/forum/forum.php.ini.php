...
include "$BASE_DIR/inc/split_long.php";
include "$BASE_DIR/inc/remove_maty.php";
include "$BASE_DIR/inc/remove_tags.php";
include "$BASE_DIR/inc/replace_links.php";
...
{
	$nid=intval($arg_nick);
	$nick=mysql_select1("select nick from nicks where id='".intval($nid)."'");
	...
	$arg_ta=mysql_escape_string($arg_ta);
 	mysql_query("insert into Forum_themes (author,name,nid,uid,ip,add_date,create_date,msgcnt,last_by,last_by_id) values('".mysql_escape_string($nick)."','".mysql_escape_string($arg_ta)."','$nid','$uid','$ip',NOW(),NOW(),'$msgc','$nick','$nid')") or die(mysql_error());
	...
	if($tid)
	{
		...
		$ip=$_SERVER['REMOTE_ADDR'];
		 mysql_query("insert into Forum_messages (author,subj,tid,nid,uid,ip,add_date,msg) values('".mysql_escape_string($nick)."','','$tid','$nid','$uid','$ip',NOW(),'".mysql_escape_string($arg_msg)."') ") or die(mysql_error());
...
if(isset($arg_rmth) && isset($arg_t) && $adm==1)
{
	$arg_t=intval($arg_t);
...
if(isset($arg_addmsg) && isset($arg_t) && $adm==1)
{
	$arg_t=intval($arg_t);
 	$arg_msg=remove_maty(split_long($arg_msg));
...
//	$arg_msg=preg_replace("/[\n\r]+/","[BR]",$arg_msg);
	...
	$_SESSION['last_nick_id']=$nid;
	$arg_t=intval($arg_t);
	$tid=mysql_select1("select id from Forum_themes where id='".intval($arg_t)."'");
	if($tid)
	{
		$nick=mysql_escape_string($nick);
		...
		$arg_msg=mysql_escape_string($arg_msg);
		$ip=$_SERVER['REMOTE_ADDR'];
		mysql_query("insert into Forum_messages (author,subj,tid,nid,uid,ip,add_date,msg) values('$nick','".mysql_escape_string($arg_subj)."','".mysql_escape_string($arg_t)."','$nid','$uid','$ip',NOW(),'".mysql_escape_string($arg_msg)."') ") or die(mysql_error());
		mysql_query("update Forum_themes set msgcnt=msgcnt+1, add_date=NOW(),last_by='".mysql_escape_string($nick)."' where id='".intval($arg_t)."'") or die(mysql_error());
	}
