...
function checked($c)
{
	...
	else if($arg_todo=="update")
	{
		...
		$result=mysql_query("select id,pic from user_status where uid='$uid' order by id") or die(mysql_error().__FILE__.__LINE__);
			...
			//echo $s;
			mysql_query("set charset cp1251");
			$sql="update user_status set name='".mysql_escape_string($s)."', pic='".mysql_escape_string($pic)."', 
			disable_invite='$disable_invite' where id='".mysql_escape_string($id)."' and uid='$uid'";
	...
	if($arg_todo=="remove")
	{
		...
		mysql_query("delete from user_status where id='".mysql_escape_string($arg_sel)."' and uid='$uid'") or die(mysql_error().__FILE__.__LINE__);
			mysql_query("insert into ext_events (event) values ('cmd=flush obj=tbl_users uid=$uid')") or die(mysql_error().__FILE__.__LINE__);
	}
	}
?>
