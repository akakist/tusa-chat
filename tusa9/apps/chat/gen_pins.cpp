...
char sss[1000000];
set<string> used;
void gen_sql(unsigned int N1, unsigned int summa1, const string& who)
{
    map<string,string,__malloc_alloc> mmm;
	...
	snprintf(xs,sizeof(xs),"('%s','%s','%s','%s','%d',NOW(),'%s')",s1.c_str(),s2.c_str(),s3.c_str(),sf.c_str(),/*rand()%*/summa1,who.c_str());
	vals.push_back(xs);
	used.insert(sf);
    }
...
/*	vector<string> vv;
	for(unsigned j=0;j<1&& i<N1;j++,i++)
	{
        ...
        printf("insert into pincodes (v1,v2,v3,full_v,summa, date_created,sent_to) values %s;\n",vals[i].c_str()/*,join(",",vv).c_str()*/);
    }
