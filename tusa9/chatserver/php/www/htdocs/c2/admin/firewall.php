...
//echo date('Y.m.d H:i:s');
        if(isset($arg_del_fw))
                {
                list($ip,$za,$uid,$time_ban,$time_free)=mysql_select1row("select ip,za,uid,time_ban,time_free from firewall where id='$arg_del_fw'");
		$tmp = "$ip $uid $time_ban $time_free $za";
		$d = time();
		$d = date('Y.m.d H:i:s');
                mysql_query("insert into adm_events (login,date_events,type_events,events) values('$login','$d','FW_DEL','$tmp')") or die(mysql_error());
	        mysql_query("delete from firewall where id='$arg_del_fw'") or die(mysql_error());
...
//            	    echo $uid $login $tmp;
            	    mysql_query("insert into adm_events (login,date_events,type_events,events) values('$login','$d','FW_SET','$tmp')") or die(mysql_error());
	    	    mysql_query("insert into firewall (ip,time_ban,time_free,time_cnt,uid,za) values('$arg_set_fw','$d','$time_free','$time_cnt','$uid','$arg_za_set')") or die(mysql_error());
