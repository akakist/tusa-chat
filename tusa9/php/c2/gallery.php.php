...
/*p{text-height:20px}*/
	...
	<form method=get   name=f>
		...
		<?
			...
			{
			$arg_n = intval($arg_n);
				$ds=mysql_select1("select nick from nicks where id='".intval($arg_n)."'");
			}
		?>
...
<?
	...
	if(isset($arg_s))
	{
	    ...
	    $sql="select a.id,a.uid,a.nick from nicks a, tbl_users b  where a.nick like '".mysql_escape_string($arg_s)."%' and a.banned=0 and a.uid<>0 and b.id=a.uid order by a.str_nick limit 100";
	    $res=mysql_query($sql);
 ...
 //       	                $d."</tr>";
	...
	//	$d="".make_user_info($arg_n,$adm)."";
		$zid=mysql_select1("select uid from nicks where id=".intval($arg_n)."");
//		echo $d;
	}
?>
</div>
...
//	$arg_n = intval($arg_n);
	...
	<?
		$result=mysql_query("select id,url_thumb_small, url_scaled ,url_host,descr from photos where uid='".intval($zid)."' order by id") or die(mysql_error());
