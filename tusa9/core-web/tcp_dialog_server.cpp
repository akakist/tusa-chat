...
string dump_tcp_dial()
{
    string out;
    {
        ...
        {
            out+="TDS: "+i->first+" = "+itostring(i->second)+"<br>\n";
        }
    }
    return out;
}
                ...
                if (g_must_terminate) return NULL;
                {
                    ...
                    if(rand()%200==0)
                    {
                        ...
                        for (map<SOCKET_id,PTR_getter<c_socket>  > ::iterator i=ccc.begin(); i!=ccc.end(); i++)
                        {
                            ...
                            if(_rm)
                            {
                                srv->on_delete(i->second,"connection lost " __FILE__ " "+itostring(__LINE__));
                            }
                        }
...
//											logErr2("(it==ccc.end()");
                                        ...
                                        {   MUTEX_INSPECTOR;
                                            ...
                                            {   MUTEX_INSPECTOR;
                                                ...
                                                {
                                                    MUTEX_INSPECTOR;
                                                    srv->on_delete(P,"connection lost " __FILE__ " "+itostring(__LINE__));
                                                    disconnect(P);
                                                    ccc.erase(fd);
                                                    continue;
                                                }
                                    ...
                                    {   MUTEX_INSPECTOR;
                                        ...
                                        {
                                            MUTEX_INSPECTOR;
                                            srv->on_delete(it->second,"connection lost " __FILE__ " "+itostring(__LINE__));
