...
//    mysql_query("insert into adm_events (login,date_events,type_events,events) values('$login','$d','LOGIN_DEL','$tmp')") or die(mysql_error());
    mysql_query("delete from bans where id='".intval($arg_del_login)."'") or die(mysql_error());
    }
...
//	    $ch = mysql_select1("select name2 from channels where id='$row[0]'");     //     Канал;
		...
		mysql_query("insert into adm_events (login,date_events,type_events,events) values('".mysql_escape_string($login)."','$d','KLAP_SET','$tmp')") or die(mysql_error());
