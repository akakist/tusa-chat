...
/*    if(uid<UNREG_BASE)
    ...
    map<string,string,malloc_alloc> mmm;
    char pn[300];
    pn[0]=0;
...
int main(int argc,char *argv[])
{
	...
	vector<vector<string> > v=dbh->exec("select refid,gender from user_sets");
    for(unsigned i=0;i<v.size();i++)
    {
	if(v[i].size()==2)
	{
	    printf("update user_profiles set gender='%s' where refid=%s;\n",v[i][1].c_str(),v[i][0].c_str());
