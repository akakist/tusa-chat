...
int main(int argc,char *argv[])
{
	...
	v=dbh->exec("select owner_uid,id,attachment_filename from notes where attachment_content_length>1");
	for(unsigned i=0;i<v.size();i++)
	{
		...
		check_path_wfn(pn2);
		int err=link(pn.c_str(),pn2.c_str());
		if(err)printf(" %s -> %s %s\n",pn.c_str(),pn2.c_str(),strerror(errno));
//		printf("rm -f %s\n",pn.c_str());
