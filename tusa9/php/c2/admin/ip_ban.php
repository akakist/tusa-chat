...
<TITLE>Чат "Тусовка" - Консоль админа -> Бан по IP</TITLE>
</head>
<?
        if(isset($arg_del_ip))
                {
                list($ip,$za,$uid,$time_ban,$time_free)=mysql_select1row("select ip,za,uid,time_ban,time_free from banned_ip where id='".mysql_escape_string($arg_del_ip)."'");
		$tmp = "$ip $uid $time_ban $time_free $za";
		$d = date('Y.m.d H:i:s');
                mysql_query("insert into adm_events (login,date_events,type_events,events) values('".mysql_escape_string($login)."','$d','IP_DEL','$tmp')") or die(mysql_error());
	        mysql_query("delete from banned_ip where id='$arg_del_ip'") or die(mysql_error());
//                echo "Файрвол снят $arg_del_ip";
...
//            	    echo $tmp;
	    	    ...
	    	    mysql_query("insert into banned_ip (ip,time_ban,time_free,time_cnt,uid,za) values('$arg_set_ip','$d','$time_free','$time_cnt','$uid','$arg_za_set')") or die(mysql_error());
            	    ...
            	    $msg="IP $arg_set_ip заблокирован";
...
<body>
<big><big><center><b>Бан по IP</b></center></big></big><br>
	...
	while($row=mysql_fetch_row($req))
	{
		...
		$dis="";
                ...
                if ($cur_base_lev>$login_ban_level || $login_ban_auth==$login || $_SESSION['level']==1000 )
                  {
                  $str_but = " class=tdrk><center> <input type=button value=\"Снять\" onclick=\"location.href='/c2/admin/ip_ban.php?del_ip=".$id."'\"$dis></center>";
                  }
                else
                  $str_but = " ";
...
?>
	...
	<form name=ip_add action="/c2/admin/ip_ban.php">
        <br>Заблокировать IP: <input type=text name="set_ip" maxlen=15 size=15 style="height: 19"> 
