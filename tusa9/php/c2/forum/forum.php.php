...
else
{
 ...
 <?
		...
		{
			$arg_t=intval($arg_t);
			list($tsize,$tname)=mysql_select1row("select msgcnt,name from Forum_themes where id='".intval($arg_t)."'");
		}	
		$page=0;
...
//			$arg_t=intval($arg_t);
	...
	<div style='text-align:center; padding:10px'>
	 ...
	 <?
	 	...
	 	if(!isset($arg_t))
		{
		 	$result=mysql_query("select add_date,id,name,msgcnt,last_by,author from Forum_themes order by add_date desc limit ".intval($start).",$FORUM_MSG_PER_PAGE") or die(mysql_error());
		...
		{
			$arg_t=intval($arg_t);
		 	$result=mysql_query("select id,ip,add_date,author,subj,msg,nid,uid from Forum_messages where tid='$arg_t' order by add_date desc limit ".intval($start).",$FORUM_MSG_PER_PAGE") or die(mysql_error());
			if(mysql_num_rows($result))
			{
